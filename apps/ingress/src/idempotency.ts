 ...
 */
export const idempotencyKey = async (
  headers: Headers,
  raw: ArrayBuffer,
  recipient: string,
): Promise<string> => {
 ...
 */
export const deriveId = async (idempotencyKey: string): Promise<string> => {
  const bytes = new TextEncoder().encode(idempotencyKey);
  return toHex(await crypto.subtle.digest("SHA-256", bytes));
};

/** Has this key already been processed? */
export const isProcessed = async (db: Db, key: string): Promise<boolean> => {
  const row = await db
...
export const markProcessed = async (db: Db, key: string): Promise<void> => {
  await db
