...
import { Button, buttonVariants, cn, Input } from "@manual.email/ui";
  ...
  return (
    <>
      <form className="flex flex-col items-start gap-2" action={action}>
        ...
        <Input
          className={cn("max-w-sm", controlClassName)}
          name="username"
      ...
      <button
        className={cn(buttonVariants({ variant: "link" }), linkClassName)}
        type="button"
      ...
      {state.error && (
        <p className={cn("text-red-700", errorClassName)} role="alert">
          {state.error}
        </p>
      )}
    </>
